...
const char* il2cpp_functions::Type_GetName(const Il2CppType *type, Il2CppTypeNameFormat format) {
    ...
    auto str = il2cpp_functions::_Type_GetName_(type, format);
    return str._M_p;
}
...
void il2cpp_functions::Init() {
    ...
    SEARCH_HOOK(MetadataCache, GetIndexForTypeDefinition,  // 0x8504BC in 1.5
        "? ? ? ? ? ? ? f9 ? ? ? ? ? ? ? f9 0a 34 40 f9 08 a1 80 b9 28 01 08 8b 48 01 08 cb 29 85 8b 52 08 fd 42 d3 e9 51 b8 72 "
        "00 7d 09 1b c0 03 5f d6");
    ...
    SEARCH_HOOK(MetadataCache, GetTypeInfoFromTypeIndex,  // 0x84F764 in 1.5
        ...
        "? ? ? ? ? ? ? f9 15 7c 40 93");
    SEARCH_HOOK(MetadataCache, GetTypeInfoFromTypeDefinitionIndex,  // 0x84FBA4 in 1.5
        "fc 6f ba a9 fa 67 01 a9 f8 5f 02 a9 f6 57 03 a9 f4 4f 04 a9 fd 7b 05 a9 fd 43 01 91 f4 03 00 2a 9f 06 00 31 ? ? ? 54 "
        "? ? ? ? ? ? ? f9 ? ? ? f8 ? ? ? b5 ? ? ? ? ? ? ? f9");
    SEARCH_HOOK(MetadataCache, GetTypeDefinitionFromIndex,  // 0x84FFBC in 1.5
        "1f 04 00 31 ? ? ? 54 ? ? ? ? ? ? ? f9 ? ? ? ? ? ? ? f9 ? ? ? b9 28 01 08 8b ? ? ? 52 00 20 29 9b c0 03 5f d6");
    SEARCH_HOOK(MetadataCache, GetExportedTypeFromIndex,  // 0x84FFF0 in 1.5
        "1f 04 00 31 ? ? ? 54 ? ? ? ? ? ? ? f9 ? ? ? ? ? ? ? f9 ? ? ? b9 28 01 08 8b 00 d9 60 b8");
    SEARCH_HOOK(MetadataCache, GetGenericContainerFromIndex,  // 0x850018 in 1.5
    ...
    SEARCH_HOOK(MetadataCache, GetNestedTypeFromIndex,  // 0x8500BC in 1.5
    ...
    SEARCH_HOOK(_Type, GetName_,  // 0x8735DC in 1.5
        "f4 4f be a9 fd 7b 01 a9 fd 43 00 91 f3 03 08 aa ? ? ? ? ? ? ? f9 e9 03 01 2a");
    SEARCH_HOOK(GenericClass, GetClass,  // 0x88DF64 in 1.5
